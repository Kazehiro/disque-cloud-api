...
import { repository } from './service.respository';
...
export default class Controller {
  ...
  public async update(req: Request, res: Response) {
    try {
      const service_id: any = req.params.id || 0;
      const check = await repository.check({ service_id: service_id });
  ...
  public async delete(req: Request, res: Response) {
    try {
      ...
      if (!check) return response.failed('data not found', 404, res);
      await repository.delete({ service_id: service_id });
...
export const services = new Controller();
