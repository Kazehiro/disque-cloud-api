...
dotenv.config();
const JWT_TOKEN: Secret = process.env.JWT_TOKEN || '';
const JWT_REFRESH_TOKEN: Secret = process.env.JWT_REFRESH_TOKEN || '';
const JWT_TOKEN_EXPIRED: string = process.env.JWT_TOKEN_EXPIRED || '0';
const JWT_REFRESH_TOKEN_EXPIRED: string =
  process.env.JWT_REFRESH_TOKEN_EXPIRED || '0';
...
export default class HelperAuth {
  ...
  public token(payload: string | Object) {
    const token: string = jwt.sign(payload, JWT_TOKEN, {
      expiresIn: parseInt(JWT_TOKEN_EXPIRED),
    });
  ...
  public refresh(payload: string | Object) {
    const refresh: string = jwt.sign(payload, JWT_REFRESH_TOKEN, {
      expiresIn: parseInt(JWT_REFRESH_TOKEN_EXPIRED),
    });
